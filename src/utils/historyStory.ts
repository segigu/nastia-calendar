...
export async function generateCustomHistoryOption({
  ...
  const systemPrompt = joinSections(
    `Ты ${author.name}, авторка интерактивной терапевтической истории во втором лице.`,
    'Твоя задача — превратить голосовое предложение пользователя в карточку выбора.',
    'Карточка должна быть реалистичной, эмпатичной и продолжать текущую историю.',
    'Ограничения:',
    '- Формат ответа: чистый JSON {"title": "...", "description": "..."} без Markdown и комментариев.',
    '- Название до 6 слов и 48 символов, без кавычек и точек.',
    '- Описание 1–2 предложения, до 140 символов, поддерживающее и конкретное.',
    '- Используй тот же тон, что и в существующих вариантах: мягкий, заботливый, с акцентом на чувства и выбор героини.',
    '- Сохраняй смысл именно того, что сказал пользователь: не выдумывай новых фактов, имён или мотивов, которых нет ни в расшифровке, ни в текущем сюжете.',
    '- Ключевые сущности (имена, предметы, действия), упомянутые пользователем, должны явно чувствоваться в итоговом тексте. Если их нет — объясни, что идея неполная и предложи перезаписать.',
    '- Не упоминай сам факт записи аудио.',
  );

  const userPrompt = joinSections(
    storyContextSection,
    summarySection,
    'РАСПОЗНАННЫЙ ТЕКСТ ПОЛЬЗОВАТЕЛЯ — СОХРАНИ СМЫСЛ БУКВАЛЬНО:',
    `"""${normalizedTranscript}"""`,
    'Сформируй вариант выбора, который вписывается в историю.',
  );
