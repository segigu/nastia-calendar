...
ОБЯЗАТЕЛЬНО:
1. Если названы КОНКРЕТНЫЕ ИМЕНА (Лея, мама, Саша) — используй ИМЕННО ЭТИ имена в сцене
2. Если названы КОНКРЕТНЫЕ ДЕЙСТВИЯ (позвонить, пойти, сказать) — покажи ИМЕННО ЭТИ действия
3. Если названы КОНКРЕТНЫЕ МЕСТА (офис, кафе, дом) — действие происходит ИМЕННО ТАМ
4. НЕ заменяй конкретику на общие слова: "Лея" НЕ = "подруга", "позвонить маме" НЕ = "обратиться за советом"
5. Сцена должна НАПРЯМУЮ показать то, что пользователь сказал — не интерпретацию, а именно ЭТО действие

ПРИМЕР ПРАВИЛЬНОЙ РАБОТЫ:
Пользователь сказал: "Пойду к Лее обсудить"
❌ НЕПРАВИЛЬНО: "Ты решила поговорить с подругой..."
✅ ПРАВИЛЬНО: "Ты идёшь к Лее. Она открывает дверь..."

Пользователь сказал: "Позвоню маме прямо сейчас"
❌ НЕПРАВИЛЬНО: "Ты решила обратиться за советом к близким..."
✅ ПРАВИЛЬНО: "Ты набираешь мамин номер. Первый гудок, второй... — Алло? — слышишь её голос."`
...
В развязке ОБЯЗАТЕЛЬНО:
1. Покажи КОНКРЕТНОЕ действие, которое она озвучила (если "позвонила маме" — покажи разговор с мамой, не "обратилась за советом")
2. Используй ТОЧНЫЕ имена/места из её слов (если "пошла к Лее" — пиши "Лея", не "подруга")
3. Развязка строится вокруг ЭТОГО КОНКРЕТНОГО действия, а не вокруг общей интерпретации

ПРИМЕР:
Сказала: "Позвоню маме"
❌ ПЛОХО: "Ты обратилась за поддержкой к близким..."
✅ ХОРОШО: "Ты позвонила маме. Услышав твой голос, она сразу поняла..."`
    : currentChoice?.transcript
...
export async function generateCustomHistoryOption({
  ...
  const systemPrompt = joinSections(
    ...
    '- Название до 6 слов и 48 символов — максимально близко к словам пользователя.',
    '- Описание 1–2 предложения, до 140 символов — сохраняй все ключевые детали.',
  );
  ...
  const userPrompt = joinSections(
    ...
    '',
    'Задача:',
    '1. Сохрани ВСЕ ключевые детали: имена, действия, места из текста пользователя',
    '2. Сделай краткое название (title): 3-6 слов, передающих суть',
    '3. Сделай развернутое описание (description): 1-2 предложения с полным смыслом',
    '4. Если текст очень краткий ("окей", "да") — немного раскрой на основе контекста истории',
    '5. Если текст конкретный ("пойду к Лее") — сохрани точно как есть',
    '',
    'Примеры:',
    'Вход: "Пойду к Лее обсудить эту ситуацию"',
    '{"title": "Пойти к Лее обсудить", "description": "Пойти к Лее и обсудить с ней эту ситуацию"}',
    '',
    ...
    '',
    'Вход: "Позвоню маме прямо сейчас"',
    '{"title": "Позвонить маме", "description": "Позвонить маме прямо сейчас"}',
  );
  ...
  const aiResponse = await callAI({
    ...
    temperature: 0.3, // Низкая температура для точного следования инструкциям
    maxTokens: 300,
    signal,
    claudeApiKey,
    claudeProxyUrl,
