...
function buildArcPrompt(args: ArcPromptArgs, psychContext?: PsychContractContext): string {
  ...
  const transcriptNote = currentChoice?.transcript && currentChoice.transcript.trim().length > 0
    ? `
Пользователь произнёс дословно: "${currentChoice.transcript.trim()}". Все ключевые факты, намерения и имена из этой фразы должны явно отразиться в сцене — нельзя перепридумывать новое решение или подменять смысл.`
    : '';
...
function buildFinalePrompt(args: FinalePromptArgs, psychContext?: PsychContractContext): string {
  ...
  const finaleTranscriptNote = currentChoice?.transcript && currentChoice.transcript.trim().length > 0
...
export async function generateCustomHistoryOption({
  ...
  const systemPrompt = joinSections(
    'Твоя задача — МИНИМАЛЬНО отформатировать текст пользователя в карточку выбора.',
    'КРИТИЧЕСКИ ВАЖНО: НЕ добавляй никаких деталей, эмоций или интерпретаций от себя!',
    '',
    '🚫 ЗАПРЕЩЕНО:',
    '- Придумывать или домысливать что-то, чего нет в тексте пользователя',
    '- Добавлять эмоциональные описания ("с болью", "радостно", "нервно")',
    '- Добавлять психологические интерпретации',
    '- Менять смысл сказанного',
    '- Добавлять имена, места или детали, которых нет в оригинале',
    '- Делать текст "литературным" или "красивым"',
    '',
    '✅ РАЗРЕШЕНО:',
    '- Немного сократить для уместности в лимиты символов',
    '- Исправить очевидные речевые ошибки распознавания',
    '- Привести к грамматически правильной форме',
    '',
    'Ограничения:',
    '- Формат ответа: чистый JSON {"title": "...", "description": "..."} без Markdown и комментариев.',
    '- Название до 6 слов и 48 символов, без кавычек и точек — используй ДОСЛОВНО слова пользователя.',
    '- Описание 1–2 предложения, до 140 символов — максимально близко к оригиналу.',
    '- Если пользователь сказал "пойду к Лее обсудить" — пиши именно "Лея", не меняй на "подруга".',
    '- Если пользователь сказал "раз два три" — так и оставь, не придумывай историю.',
  );
  ...
  const userPrompt = joinSections(
    ...
    summarySection,
    '',
    'РАСПОЗНАННЫЙ ТЕКСТ ПОЛЬЗОВАТЕЛЯ (используй его ДОСЛОВНО):',
    `"""${normalizedTranscript}"""`,
    '',
    'Инструкция:',
    '1. Прочитай текст пользователя',
    '2. Сделай из него краткое название (title) — просто перефразируй в 3-6 слов',
    '3. Сделай чуть более полное описание (description) — расшифруй в 1-2 предложения',
    '4. НЕ добавляй ничего от себя — только то, что ЯВНО сказал пользователь',
    '',
    'Примеры ПРАВИЛЬНОЙ работы:',
    'Вход: "Пойду к Лее обсудить эту ситуацию"',
    'Выход: {"title": "Пойти к Лее", "description": "Обсудить ситуацию с Леей"}',
    '',
    'Вход: "Промолчу и сделаю вид что ничего не случилось"',
    'Выход: {"title": "Промолчать", "description": "Сделать вид, что ничего не случилось"}',
    '',
    'Вход: "раз два три"',
    'Выход: {"title": "Раз два три", "description": "Раз два три"}',
  );
