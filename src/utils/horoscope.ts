...
const SUBTLE_HINT_FRAMES = [
  ...
  // Фрейм 7: КОНТРОЛЬ И МЕРА (про невозможность остановиться)
  {
    type: 'control',
    hints: [
      'Одного бокала достаточно? Или снова «ещё чуть-чуть» превращается в «где я, какой день»?',
      'Когда обещаешь себе «только один раз» и каждый раз нарушаешь — это не слабость, это сигнал.',
      'Контроль — это когда можешь остановиться. Если не можешь, значит уже не ты контролируешь.',
      'Вечер начинается с «расслаблюсь чуть-чуть», а заканчивается провалами в памяти. Это норма?',
      'Обещать себе меру и каждый раз срываться — это не просто привычка, это зависимость.',
    ],
  },
  // Фрейм 8: УТРО ПОСЛЕ (похмелье, стыд, последствия)
  {
    type: 'morning_after',
    hints: [
      'Утренняя тошнота и стыд за вчерашнее — это не «весело провели время», это цена.',
      'Каждое утро с головной болью и провалами в памяти — организм кричит «хватит».',
      'Стыдно вспоминать, что говорила вчера? Может, пора остановиться, пока есть что вспоминать.',
      'Провалы в памяти — это не смешно, это мозг отключается от перегрузки. Он не железный.',
      'Утром обещаешь «больше никогда», вечером снова повторяется. Где тут ты настоящая?',
    ],
  },
  // Фрейм 9: ВЛИЯНИЕ НА СЕРГЕЯ (он устаёт, он видит)
  {
    type: 'partner_alcohol',
    hints: [
      'Серёжа видит твоё состояние утром, видит, как меняешься вечером. Он не говорит, но устаёт.',
      'Партнёр не пьёт — для него твоё «немного расслабиться» выглядит иначе. Он считает бокалы молча.',
      'Когда ты пьяная, Серёжа становится няней, а не партнёром. Романтика? Нет, обязанность.',
      'Он терпит твои вечера с бутылкой, но это не значит, что ему нравится. Просто любит.',
      'Серёжа не поддерживает эту привычку — он адаптировался к ней. Разница понимаешь?',
    ],
  },
  // Фрейм 10: ЗАМЕНА ЧУВСТВ (пить от стресса, скуки, проблем)
  {
    type: 'emotional_escape',
    hints: [
      'Пить от стресса — это как лечить температуру обезболивающим. Симптом уйдёт, болезнь останется.',
      'Бутылка не решает проблемы, она их откладывает. Завтра всё то же самое, только с головной болью.',
      'Скучно — и сразу тянет выпить? Это не расслабление, это неумение быть наедине с собой.',
      'Каждый раз заглушать чувства алкоголем — значит не проживать жизнь, а убегать от неё.',
      'Когда эмоции слишком сильные, хочется их залить. Но они никуда не денутся, только накопятся.',
    ],
  },
  // Фрейм 11: ЗДОРОВЬЕ И ВНЕШНОСТЬ (печень, кожа, старение)
  {
    type: 'health_appearance',
    hints: [
      'Печень молчит долго, но когда заговорит — будет поздно. Боль появляется, когда уже критично.',
      'Мешки под глазами, отёки, тусклая кожа — это не возраст, это организм не справляется с токсинами.',
      'Алкоголь старит быстрее времени. Посмотри в зеркало утром — это ты или её бледная копия?',
      'Давление скачет, сердце стучит не в ритм — это не стресс, это последствия вчерашнего.',
      'Тело прощает первые разы, потом начинает вести счёт. И однажды выставит счёт.',
    ],
  },
  // Фрейм 12: ЧЕСТНОСТЬ С СОБОЙ (признать проблему)
  {
    type: 'self_honesty',
    hints: [
      'Говоришь «я контролирую» — но каждый вечер повторяется одно и то же. Где тут контроль?',
      'Признать проблему — не слабость, а первый шаг к свободе. Отрицание — вот настоящая слабость.',
      'Сколько раз обещала себе «последний раз»? Если больше трёх — пора признать, что не последний.',
      'Оправдания типа «все так делают» — это самообман. Твоё тело, твоя жизнь, твой выбор.',
      'Честно ответь себе: ты пьёшь, потому что хочешь, или потому что не можешь иначе?',
    ],
  },
];
