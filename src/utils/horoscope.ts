...
const SUBTLE_HINT_FRAMES = [
  ...
  // Фрейм 3: ЗДОРОВЬЕ И КАШЕЛЬ (про лёгкие, болезни, последствия)
  {
    type: 'health',
    hints: [
  ...
  // Фрейм 4: СВЕЖЕСТЬ И ЗАПАХИ (про близость, интимность, ощущения)
  {
    type: 'freshness',
    hints: [
      ...
      'Утренний поцелуй должен быть приятным, а не испытанием. Если партнёр терпит — это не значит, что ему нравится.',
  ...
  // Фрейм 5: ВРЕМЯ И РИТУАЛЫ (про паузы, потерянное время, навязчивость)
  {
    type: 'time',
    hints: [
  ...
  // Фрейм 6: ЯСНОСТЬ И ТУМАН (про состояния ума)
  {
    type: 'clarity',
    hints: [
  ...
  // Фрейм 7: УТРЕННИЕ ОЩУЩЕНИЯ (про последствия без называния причины)
  {
    type: 'morning_state',
    hints: [
  ...
  // Фрейм 8: СПОСОБЫ УБЕЖАТЬ (про бегство от чувств)
  {
    type: 'escape_methods',
    hints: [
  ...
  // Фрейм 9: ОТРАЖЕНИЕ В ЗЕРКАЛЕ (про внешность и состояние)
  {
    type: 'reflection',
    hints: [
  ...
  // Фрейм 10: ОБЕЩАНИЯ СЕБЕ (про паттерн повторений)
  {
    type: 'self_promises',
    hints: [
...
Живёт с партнёром Сергеем (13 декабря 1979). Упоминай его ТОЛЬКО если есть реальный повод, связанный с бытом, конфликтом или поддержкой. НЕ вставляй для галочки.
Не выдумывай других родственников, детей, подруг и т.д. Если нет свежего повода для упоминания партнёра — говори о текущих ощущениях Насти, её делах, планах, погоде, настроении, цикле.
`.trim();
...
- Фокус: её настроение, бытовые задачи, планы, собственное тело и циклы.
- Упоминай партнёра Сергея ТОЛЬКО если есть конкретный повод (конфликт, поддержка, совместные планы). НЕ вставляй его для галочки типа "зато Сереже".
...
- Фокус: дела дня, настроение, бытовая рутина, тело и ощущения.
- Упоминай Серёжу ТОЛЬКО если есть реальный повод. НЕ используй шаблоны типа "зато Сереже".
