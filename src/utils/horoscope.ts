...
export const SERGEY_LOADING_FALLBACK: HoroscopeLoadingMessage[] = [
  ...
  { emoji: '🪑', text: 'Венера сдвигает стул: Серёжа опять заседает, делая вид, что всё под контролем.' },
  { emoji: '🧊', text: 'Нептун кладёт лёд на стол — чтобы Серёжа остыл, прежде чем ещё что-нибудь сломает.' },
  { emoji: '📡', text: 'Уран ловит сигнал — вдруг Серёжа хоть раз ответит без трёх напоминаний.' },
  { emoji: '📉', text: 'Солнце сверяет график падений — Серёжа уже отметил, где сегодня нырнуть в хаос.' },
  { emoji: '🪤', text: 'Плутон ставит капканы: Серёжа слишком бодро идёт, пора бы ему снова встрять.' },
];
...
Каждый статус должен:
- начинаться с одного подходящего эмодзи и пробела;
- состоять ровно из двух предложений, каждое по 8-10 слов;
- рассказывать, что Серёжа якобы занят, но на деле косячит, с отсылками к планетам или космосу;
- звучать саркастично, допускается лёгкий жаргон, но без открытой нецензурщины;
- не повторять шутки и формулировки друг друга.
...
Верни строго JSON-массив объектов вида [{"emoji":"✨","text":"..."}] без пояснений.`;
  ...
  try {
    ...
    return SERGEY_LOADING_FALLBACK;
  }
}
...
- subtitle — 1-2 предложения, где с лёгким сарказмом подмечают, что Серёжа снова мутит что-то, и предлагают взглянуть его гороскоп именно на сегодня.
