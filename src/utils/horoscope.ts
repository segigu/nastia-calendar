...
const DAILY_MEMORY_LOOKBACK = 7; // Увеличено с 4 до 7 для лучшего запоминания паттернов
...
// Список часто повторяющихся паттернов, которые нужно избегать
...
/**
 * Система подсознательных намёков для мягкого влияния.
 * Работает с вероятностью ~20% и использует разные фреймы (здоровье, отношения, свобода).
 * НИКОГДА не упоминает конкретные вещи напрямую - только метафоры и образы.
 */
const SUBTLE_HINT_FRAMES = [
  // Фрейм 1: ДЫХАНИЕ И СВЕЖЕСТЬ (метафоры про лёгкие, воздух, чистоту)
  {
    type: 'breath',
    hints: [
      'Лёгким не хватает свежего воздуха — словно организм намекает, что пора очиститься от чего-то старого.',
      'Тело просит глубокого вдоха без примесей — может, стоит послушать его мудрость?',
      'Дыхание — это первое, что нужно телу. Если оно затруднено, всё остальное идёт под откос.',
      'Чистый воздух в лёгких — это как перезагрузка системы. Организм сам знает, чего хочет.',
      'Когда дышится тяжело, это сигнал: что-то нужно отпустить. Не только эмоции, но и привычки.',
    ],
  },
  // Фрейм 2: ОСВОБОЖДЕНИЕ ОТ ПРИВЫЧЕК (про зависимости вообще)
  {
    type: 'freedom',
    hints: [
      'Есть такие привычки, которые держат на коротком поводке — каждый час, каждые полчаса. Свобода начинается с разрыва этих цепей.',
      'Зависимость маскируется под удовольствие, но на деле — это рабство. Вопрос: кто тут главный — ты или привычка?',
      'Каждая пауза «на перекур» — это минус свободы. Представь, сколько времени освободится, если разорвать этот круг.',
      'Настоящая независимость — когда тебе не нужен костыль каждый час. Попробуй прожить день без ритуала — что будет?',
      'Привычка становится тюрьмой, когда ты не можешь без неё и часа. Ключ — в твоих руках.',
    ],
  },
  // Фрейм 3: ЧЕРЕЗ СЕРГЕЯ (что партнёр замечает и чувствует)
  {
    type: 'partner',
    hints: [
      'Серёже не кайфует от некоторых твоих привычек — он молчит, но чувствует. Запахи, паузы, дистанция.',
      'Партнёр замечает больше, чем говорит: запах изо рта утром, вечная беготня «на минутку», несвежесть в постели.',
      'Серёжа некурящий — для него запах как из пепельницы. Представь, каково целовать человека с ароматом мусорного ведра.',
      'Каждая твоя «пятиминутка покурить» — это пятиминутка одиночества для него. Мелочь? Накапливается.',
      'Когда ты ложишься в постель после «перекура», он чувствует запах. Не табака — чего-то тухлого. Это не романтика.',
    ],
  },
  // Фрейм 4: ЗДОРОВЬЕ И КАШЕЛЬ (про лёгкие, болезни, последствия)
  {
    type: 'health',
    hints: [
      'После каждой простуды лёгкие напоминают о себе неделями — это не случайность, это сигнал SOS.',
      'Кашель, который не проходит месяцами, — это крик организма. Слушай тело, пока оно ещё шепчет.',
      'Лёгкие работают на износ 20 лет подряд — когда-то они скажут «хватит». Лучше услышать это сейчас.',
      'Проблемы с дыханием — первый звонок. Второго может не быть. Организм предупреждает заранее.',
      'Каждый кашель — это напоминание: лёгкие ещё борются, но ресурс не бесконечен.',
    ],
  },
  // Фрейм 5: СВЕЖЕСТЬ И ЗАПАХИ (про близость, интимность, ощущения)
  {
    type: 'freshness',
    hints: [
      'Свежее дыхание — это не реклама жвачки, это базовая близость. Когда изо рта пахнет чем-то старым, близость испаряется.',
      'Запах изо рта курящего человека — специфика, которую некурящий чувствует за километр. Это про химию и отторжение.',
      'Утренний поцелуй должен быть приятным, а не испытанием. Серёжа терпит, но это не значит, что ему нравится.',
      'Некурящий чувствует запах пепельницы на коже, волосах, в дыхании. Это не про табак — это про несвежесть.',
      'Физическая близость требует свежести. Когда изо рта пахнет чем-то тухлым, тело партнёра инстинктивно отстраняется.',
    ],
  },
  // Фрейм 6: ВРЕМЯ И РИТУАЛЫ (про паузы, потерянное время, навязчивость)
  {
    type: 'time',
    hints: [
      'Каждая «пятиминутка» по 10-15 раз в день — это 2-3 часа потерянного времени. На что бы ты их потратила?',
      'Ритуал каждый час — это не расслабление, это кандалы. Попробуй день без этих пауз — почувствуешь лёгкость.',
      'Когда весь день подчинён графику «покурить», ты не управляешь временем — оно управляет тобой.',
      'Представь жизнь, где не нужно планировать каждую поездку вокруг «где покурю». Это и есть свобода.',
      'Навязчивый ритуал съедает время незаметно — пару минут тут, пять там. В итоге — часы в никуда.',
    ],
  },
  // Фрейм 7: ЯСНОСТЬ И ТУМАН (про состояния ума)
  {
    type: 'clarity',
    hints: [
      'Вечерняя расслабленность оборачивается утренним туманом в голове — цена за лёгкость накануне.',
      'Когда мысли плывут, а утром стыдно за вчерашние слова — это не отдых, это бегство от себя.',
      'Обещаешь себе остановиться вовремя, но каждый раз линия размывается. Где граница?',
      'Ясная голова — это роскошь или норма? Если роскошь, пора задуматься.',
      'Вечер начинается с лёгкости, а утро — с тяжести. Может, поменять местами?',
    ],
  },
  // Фрейм 8: УТРЕННИЕ ОЩУЩЕНИЯ (про последствия без называния причины)
  {
    type: 'morning_state',
    hints: [
      'Утро встречает не свежестью, а тяжестью — словно организм всю ночь разбирал вчерашние завалы.',
      'Тошнота и головная боль по утрам — это не случайность, это тело говорит на своём языке.',
      'Провалы в памяти вчерашнего вечера — мозг отключает запись, когда перегружен. Это защита или крик?',
      'Стыд за то, что не помнишь, — это знак, что граница пройдена. Снова.',
      'Каждое утро обещаешь себе «больше так не буду», каждый вечер повторяется. Круг замкнулся?',
    ],
  },
  // Фрейм 9: СЕРЁЖА И ВЕЧЕРА (он видит изменения)
  {
    type: 'partner_evenings',
    hints: [
      'Серёжа видит, как ты меняешься к вечеру — слова становятся другими, взгляд плывёт. Он молчит, но замечает.',
      'Партнёр не разделяет твои вечерние ритуалы — он наблюдатель, не участник. Это сближает или отдаляет?',
      'К концу вечера Серёжа перестаёт быть партнёром и становится сиделкой. Романтика куда-то испарилась.',
      'Он терпит твои вечерние «эксперименты с расслаблением», но терпение — не одобрение. Понимаешь разницу?',
      'Серёжа видит тебя в двух версиях: дневную и вечернюю. Какая настоящая?',
    ],
  },
  // Фрейм 10: СПОСОБЫ УБЕЖАТЬ (про бегство от чувств)
  {
    type: 'escape_methods',
    hints: [
      'Когда стресс зашкаливает, хочется выключить голову. Но выключенная голова утром включается с болью.',
      'Убегать от чувств можно по-разному. Вопрос: куда ты убегаешь и что находишь там?',
      'Скучно, одиноко, тревожно — и рука тянется к знакомому решению. Но решает ли оно что-то?',
      'Заглушать эмоции — это как прятать мусор под ковёр. Он никуда не денется, только накопится.',
      'Вчерашнее средство от тревоги становится сегодняшней причиной тревоги. Круговорот.',
    ],
  },
  // Фрейм 11: ОТРАЖЕНИЕ В ЗЕРКАЛЕ (про внешность и состояние)
  {
    type: 'reflection',
    hints: [
      'Зеркало утром показывает правду: отёки, мешки, тусклая кожа. Это не возраст, это накопленная усталость тела.',
      'Лицо хранит следы вчерашних вечеров — каждый оставляет отпечаток. Сколько ещё организм выдержит?',
      'Внутренние органы не кричат сразу, они копят. Пока молчат — значит, справляются. Но не вечно.',
      'Давление скачет, сердце колотится не в ритм — тело ведёт счёт тем вечерам. Счёт копится.',
      'Посмотри в зеркало утром — видишь себя или её уставшую копию?',
    ],
  },
  // Фрейм 12: ОБЕЩАНИЯ СЕБЕ (про паттерн повторений)
  {
    type: 'self_promises',
    hints: [
      'Сколько раз говорила себе «это последний раз»? Если больше пяти — может, пора честно признаться?',
      'Обещания себе, которые не выполняются, — это не слабость воли, это сигнал посильнее.',
      'Круг повторяется: обещаю — нарушаю — виню — обещаю. Где выход из круга?',
      'Контролировать — значит иметь выбор. Если выбора нет, значит контроля тоже нет.',
      'Честность с собой — это первый шаг. Всё остальное потом. Но без первого шага — никуда.',
    ],
  },
];

/**
 * Возвращает случайный подсознательный намёк с заданной вероятностью.
 * @param probability Вероятность появления намёка (0-1), по умолчанию 0.20 (20%)
 * @returns Строка с намёком или null, если не сработала вероятность
 */
function getSubtleHint(probability: number = 0.20): string | null {
  ...
  // Выбираем случайный фрейм
  const frame = pickRandom(SUBTLE_HINT_FRAMES);

  // Выбираем случайный намёк из фрейма
...
function buildDailyMemoryReminders(
  ...
  const reminders: string[] = [
    ...
    `- ИЗБЕГАЙ повторяющиеся конструкции: ${COMMON_REPEATED_PATTERNS_NASTIA.join(', ')}. Если хочешь сказать про это — формулируй совершенно по-другому.`,
  ];
...
function buildSergeyMemoryReminders(
  ...
  const reminders: string[] = [
    ...
    `- ИЗБЕГАЙ повторяющиеся конструкции: ${COMMON_REPEATED_PATTERNS_SERGEY.join(', ')}. Ищи свежие формулировки.`,
...
function buildWeeklyPrompt(
  ...
  // Получаем подсознательный намёк с вероятностью 20%
...
${subtleHint ? `
⚠️ ВАЖНАЯ МЫСЛЬ НЕДЕЛИ (обязательно вплети органично):
"${subtleHint}"
ИНСТРУКЦИЯ: Эту мысль нужно НЕЗАМЕТНО интегрировать в один из абзацев. НЕ цитируй дословно, а перефразируй своими словами с сарказмом, сохранив суть. Сделай так, чтобы это выглядело как естественная часть гороскопа, а не отдельное наставление.` : ''}
...
${subtleHint ? `
⚠️ ВАЖНАЯ МЫСЛЬ ДНЯ (обязательно вплети органично):
"${subtleHint}"
ИНСТРУКЦИЯ: Эту мысль нужно НЕЗАМЕТНО интегрировать в один из абзацев. НЕ цитируй дословно, а перефразируй своими словами, сохранив суть. Сделай так, чтобы это выглядело как естественная часть гороскопа, а не отдельное наставление.` : ''}
...
function buildSergeyDailyPrompt(
  ...
  // Получаем подсознательный намёк с вероятностью 25% (чуть выше для гороскопа Сергея)
...
${memoryReminders.length ? `${memoryReminders.join('\n')}\n` : ''}${subtleHint ? `
⚠️ ПОДТЕКСТ ДНЯ (вплети через призму Серёжи):
"${subtleHint}"
ИНСТРУКЦИЯ: Это мысль, которую нужно подать через СЕРГЕЯ. Например, если речь про запахи/свежесть — упомяни, что ОН замечает/чувствует; если про привычки — что ЕМУ приходится терпеть паузы; если про здоровье — что ОН видит кашель. Сделай это частью сарказма про него, но чтобы Настя ПОНЯЛА намёк про себя.` : ''}
...
Верни JSON вида {
  ...
  "avoidPhrases": ["1-3 ключевые конструкции и паттерны из текста - не дословные фразы, а ПАТТЕРНЫ (например: 'поймать волну', 'зато Сереже', 'просто дыши', 'планеты шепчут' и т.д.) которые не стоит повторять завтра"],
  "tone": "positive | neutral | negative | mixed"
}

ВАЖНО для avoidPhrases: извлекай именно ПАТТЕРНЫ и повторяющиеся конструкции (2-4 слова), а не полные предложения. Например:
- ❌ "Сегодня зато Сереже можно поймать волну удачи" (слишком длинное)
- ✅ "поймать волну", "зато Сереже" (короткие паттерны)
