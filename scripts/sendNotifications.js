...
Сегодня: ${context.todayHuman}. Прогноз старта менструации: ${context.periodHuman}.`;
  ...
  switch (type) {
    case 'fertile_window':
      return `${base}
Ситуация: фертильное окно, до овуляции ${Math.abs(context.daysUntilOvulation)} ${context.daysWord}. Предупреди про риск беременности с юмором и сарказмом. Максимум 120 символов!`;
    case 'ovulation_day':
      return `${base}
Ситуация: сегодня овуляция (пик фертильности). Саркастично предупреди о высоком риске беременности. Максимум 120 символов!`;
    case 'period_forecast':
      return `${base}
Ситуация: до менструации ${Math.abs(context.daysUntilPeriod)} ${context.daysWord}. Напомни что ПМС скоро, дай совет как пережить (будь креативна). Максимум 120 символов!`;
    case 'period_check':
    case 'period_start':
      return `${base}
Ситуация: прогноз на ${context.periodHuman} — то есть сегодня. Попроси проверить, началась ли менструация, и отметить в календаре. Максимум 120 символов!`;
    case 'period_waiting':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord} после прогноза. Поддержи саркастично, скажи что организм сам разберётся. Максимум 120 символов!`;
    case 'period_delay_warning':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Саркастично намекни что неплохо бы сделать тест на беременность. Максимум 120 символов!`;
    case 'period_confirmed_day0':
      return `${base}
Ситуация: Настя отметила начало менструации сегодня (день 1). Поддержи с юмором и сарказмом. Максимум 120 символов!`;
    case 'period_confirmed_day1': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи с юмором, дай совет (будь креативна). Максимум 120 символов!`;
    }
