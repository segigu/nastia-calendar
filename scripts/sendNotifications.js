...
const responseSchema = {
  ...
  schema: {
    ...
    properties: {
      ...
      body: {
        type: 'string',
        description: 'Push body up to 80 characters with 1-2 emojis and biting supportive sarcasm.',
        maxLength: 85,
      },
    },
  },
...
const fallbackMessages = {
  ...
  period_check: {
    ...
    body: 'Настюх, день Х сегодня — проверься и отметь, если началось. 👀',
  },
  period_waiting: {
    title: 'Глаша Терпеливая',
    body: 'Настёна, задержка день — прислушайся к себе и отметь в дневник. 🤔',
  },
  ...
  birthday: {
    ...
    body: 'Настюш, с днюхой! Устроим праздник и ноль драмы. 🎉💜',
  },
  morning_brief: {
...
function truncateWithEllipsis(text, limit = 80) {
...
Формат JSON:
{
  ...
  "body": "жёсткая саркастичная строка до 70 символов с 1-2 эмодзи, обращение к Насте"
}
...
- В тексте прямо намекни, что это гороскоп на сегодня (используй формулировки вроде «гороскоп на день», «сегодняшний гороскоп», «гороскоп на сейчас»).
- Заверши body призывом заглянуть внутрь и узнать подробности.
- Чередуй подачу: то юмор, то лёгкая тревога, то уверенная поддержка; не зацикливайся на словах «хаос», «экзамен» и «дети орут».
- Заголовок тоже варьируй: иногда начинать с «Гороскоп…», иногда с образа или действия, но всегда чтобы хотелось открыть.
...
- Тело до 70 символов с обращением к Насте, 1-2 эмодзи и жёстким, но заботливым сарказмом. Пиши от лица персонажа из заголовка, будто он шлёт сообщение в чат. Будь краток и ёмок. Никакой мягкости, но и без обсценной лексики и унижений.
...
Ситуация: ${dayWord} менструации (старт ${context.periodStartHuman}). Поддержи, напомни про спокойный режим и намекни, что Настя всё держит под контролем.`;
    ...
    case 'birthday':
      return `Ты — Настина лучшая подруга с жёстким, но поддерживающим сарказмом. Пиши по-русски, формат push: новый персонаж в заголовке (1-3 слова) и короткое тело до 70 символов с 1-2 эмодзи.
Сегодня ${context.todayHuman} и Настюше исполняется ${context.birthdayAge}. Поздравь её дерзко и тепло, будто подруга шлёт сообщение в чат. Будь краток. Без официоза и пафоса, только живая язвительная любовь. ЗАПРЕЩЕНО использовать имена: Игорь, Константин, Стас.`;
    case 'morning_brief':
      return `Ты — Настина лучшая подруга с чёрным юмором. Формат push: новый персонаж в заголовке (1-3 слова), тело до 70 символов, 1-2 эмодзи, острый сарказм допустим даже с лёгкими матами.
