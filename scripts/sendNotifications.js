...
3. Body: СТРОГО до 50 символов (с эмодзи)! Это КРИТИЧНО - телефон обрежет длинное.

   КАК уложиться в 50 символов:
   - Убери вводные слова: НЕ "сегодня будет", а сразу "конфликты — держись!"
   - Одна мысль: либо что будет, либо что делать
   - Короткие слова: "терпи" вместо "придётся потерпеть"

   Примеры ПРАВИЛЬНЫЕ (≤50 символов, законченная мысль):
   - "Настя, конфликты весь день — держись! 😤" (40)
   - "Детвора орёт, терпи до вечера! 💪" (34)
   - "Днём напряг, вечером легче! 🔥" (30)
   - "Слушай себя — не прогадаешь! ✨" (31)

   Примеры НЕПРАВИЛЬНЫЕ (слишком длинно или про планеты):
   - ❌ "Настя, сегодня Марс против Сатурна — держись крепче!" (52 - длинно!)
   - ❌ "Детвора бесится, экзамен давит — как-то терпи!" (48 - можно короче!)
   - ❌ "Венера в квадрате с Луной!" (астрология)

4. НЕ используй астрологические термины в body! Только про ЖИЗНЬ: конфликты, отдых, дела, интуицию, детей, Серёжу, экзамен.
5. ПОСЧИТАЙ символы ПЕРЕД отправкой! Если >50 - сократи фразу.
...
7. Ответ — строго JSON без дополнительного текста.`;
  ...
  const systemPrompt = 'Ты пишешь острые push-уведомления. Отвечай только JSON-объектом. КРИТИЧНО: body СТРОГО до 50 символов (включая эмодзи)! Переводи астрологию на простой язык. Убирай вводные слова, пиши короткими фразами. ОБЯЗАТЕЛЬНО посчитай символы перед отправкой!';
...
- Тело СТРОГО до 50 символов (с эмодзи!) с обращением к Насте. Пиши от лица персонажа из заголовка, будто он шлёт сообщение в чат. Будь КРАТОК - убирай вводные слова, пиши одну законченную мысль.

КРИТИЧНО: body максимум 50 символов! Телефон обрежет длинное. ПОСЧИТАЙ символы перед отправкой!
Сегодня: ${context.todayHuman}. Прогноз старта менструации: ${context.periodHuman}.`;

  switch (type) {
    case 'fertile_window':
      return `${base}
Ситуация: фертильное окно, до овуляции ${Math.abs(context.daysUntilOvulation)} ${context.daysWord}. Предупреди про риск залёта коротко и ясно. Максимум 50 символов!

Примеры: "Настюш, риск залёта — защищайся! 💋" (36), "Зона риска, контрацепция обязательна! 🛡️" (41)`;
    case 'ovulation_day':
      return `${base}
Ситуация: сегодня овуляция. Скажи про пик фертильности коротко. Максимум 50 символов!

Примеры: "Настёна, овуляция — прикрывайся! 🔥" (35), "Пик фертильности, защищайся! 💥" (33)`;
    case 'period_forecast':
      return `${base}
Ситуация: до менструации ${Math.abs(context.daysUntilPeriod)} ${context.daysWord}. Напомни, что ПМС скоро. ВАРЬИРУЙ совет (грелка, шоколад, терпение, запасы, отдых - чередуй!). Максимум 50 символов!

Примеры: "Настюх, через ${Math.abs(context.daysUntilPeriod)} ${context.daysWord} шторм — запасайся! 🙄" (≤50), "Скоро ПМС, готовь шоколад! 🍫" (30)`;
    case 'period_check':
    case 'period_start':
      return `${base}
Ситуация: прогноз на ${context.periodHuman} — то есть сегодня. Попроси проверить, началось ли. Максимум 50 символов!

Примеры: "Настюх, день Х — проверься! 👀" (28), "Прогноз сегодня, отметь старт! 📝" (32)`;
    case 'period_waiting':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Поддержи коротко. Максимум 50 символов!

Примеры: "Настёна, задержка — прислушайся! 🤔" (35), "Уже ${context.daysPastPrediction} ${context.daysPastPredictionWord} тянет, как дела? 💭" (≤50)`;
    case 'period_delay_warning':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Намекни про тест коротко. Максимум 50 символов!

Примеры: "Настюш, ${context.daysPastPrediction} ${context.daysPastPredictionWord} тянет — может, тест? 😬" (≤50), "Задержка долгая, проверься! 🧪" (29)`;
    case 'period_confirmed_day0':
      return `${base}
Ситуация: Настя отметила начало менструации сегодня. Поддержи коротко. Максимум 50 символов!

Примеры: "Настёна, старт! Грелка и покой 🛋️" (33), "Началось, береги себя! 💜" (25)`;
    case 'period_confirmed_day1': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи коротко. Максимум 50 символов!

Примеры: "Настюш, день 2 — грелка к пузику! 🔥" (35), "Второй день, держись! 💪" (24)`;
    }
...
Примеры: "Настёна, день 3, пей воду! 💪" (28), "Третий день, скоро полегчает! 🌸" (34)`;
    }
    case 'birthday':
      return `Ты — Настина лучшая подруга с жёстким, но поддерживающим сарказмом. Пиши по-русски, формат push: новый персонаж в заголовке (1-3 слова) и тело СТРОГО до 50 символов с 1-2 эмодзи.
Сегодня ${context.todayHuman} и Настюше исполняется ${context.birthdayAge}. Поздравь дерзко и тепло коротко. ЗАПРЕЩЕНО использовать имена: Игорь, Константин, Стас.

Примеры: "Настюш, ${context.birthdayAge} тебе! Праздник! 🎉💜" (≤50), "С днюхой, Орлова! Гуляй! 🥳" (28)`;
    default:
      return base;
  }
}
