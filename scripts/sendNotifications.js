...
const fallbackMessages = {
  fertile_window: {
    title: 'Настёнка, внимание',
    body: '💋 Настюш, фертильное окно открыто. Если не хочешь мини-версию — план Б держи под рукой.',
  },
  ovulation_day: {
    title: 'Настюша, день особенный',
    body: '🔥 Настён, сегодня овуляция. Презервативы не для красоты, а для спокойствия твоего.',
  },
  period_forecast: {
    title: 'Настюшка, готовься',
    body: '🙄 Настёна, до менструации пару дней. Шоколад, грелку и терпение — всё это пригодится.',
  },
  period_start: {
    title: 'Настёнок, день настал',
    body: '🩸 Настюх, менструация началась. Укутайся и береги себя, никого не подпускай без чая.',
  },
};
...
function buildPrompt(type, context) {
  const base = `Ты — близкая подруга Насти, с тёплым чувством юмора и заботой. Пиши по-русски легко и разговорно, обращайся к Насте нежно (Настюш, Настён, Настёнка, Настюшка, Настёна).
Задача: придумать push-уведомление для календаря цикла.
Формат: короткий заголовок до 5 слов (обязательно с обращением по имени) и тело до 110 символов с обращением. В теле 1-2 подходящих эмодзи. Тон — тёплый, поддерживающий, с лёгкой иронией. Без обсценной лексики, без унижений.
Сегодня: ${context.todayHuman}. Следующая менструация: ${context.periodHuman}.`;

  switch (type) {
    case 'fertile_window':
      return `${base}
Ситуация: фертильное окно, до овуляции ${context.daysUntilOvulation} ${context.daysWord}. Нежно, но чётко предупреди про вероятность беременности и важность защиты.`;
    case 'ovulation_day':
      return `${base}
Ситуация: сегодня овуляция. Сообщи об этом ласково, но прямо, с напоминанием про контрацепцию.`;
    case 'period_forecast':
      return `${base}
Ситуация: до менструации ${context.daysUntilPeriod} ${context.daysWord}. Поддержи тепло и с пониманием: лёгкая ирония про ПМС, совет позаботиться о себе.`;
    case 'period_start':
      return `${base}
Ситуация: ожидается начало менструации сегодня. Тёплое, заботливое сообщение с советом беречь себя и создать комфорт.`;
    default:
      return base;
  }
}
