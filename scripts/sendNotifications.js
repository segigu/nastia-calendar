...
- Тело: ОДНА короткая фраза, МАКСИМУМ 95 символов (с эмодзи!). Пиши ОЧЕНЬ лаконично от лица персонажа. Юмор + сарказм. Допустим мат.
...
Сегодня: ${context.todayHuman}. Прогноз менструации: ${context.periodHuman}.`;
  ...
  switch (type) {
    case 'fertile_window':
      return `${base}
Ситуация: фертильное окно, до овуляции ${Math.abs(context.daysUntilOvulation)} ${context.daysWord}. Предупреди про риск беременности с юмором. ОДНА фраза, макс 95 символов!`;
    case 'ovulation_day':
      return `${base}
Ситуация: овуляция сегодня (пик фертильности). Саркастично предупреди о риске. ОДНА фраза, макс 95 символов!`;
    case 'period_forecast':
      return `${base}
Ситуация: до менструации ${Math.abs(context.daysUntilPeriod)} ${context.daysWord}. Напомни про ПМС, дай короткий совет. ОДНА фраза, макс 95 символов!`;
    case 'period_check':
    case 'period_start':
      return `${base}
Ситуация: прогноз менструации на ${context.periodHuman} (сегодня). Попроси проверить и отметить. ОДНА фраза, макс 95 символов!`;
    case 'period_waiting':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Поддержи саркастично. ОДНА фраза, макс 95 символов!`;
    case 'period_delay_warning':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Намекни про тест на беременность с сарказмом. ОДНА фраза, макс 95 символов!`;
    case 'period_confirmed_day0':
      return `${base}
Ситуация: Настя отметила начало менструации сегодня. Поддержи с юмором. ОДНА фраза, макс 95 символов!`;
    case 'period_confirmed_day1': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи с юмором, короткий совет. ОДНА фраза, макс 95 символов!`;
    }
    case 'period_confirmed_day2': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи, скажи что скоро легче. ОДНА фраза, макс 95 символов!`;
    }
    case 'birthday':
      return `Ты — Настина подруга с жёстким сарказмом. Push-уведомление: новый персонаж в заголовке (1-3 слова), тело ОДНА фраза макс 95 символов с 1-2 эмодзи.
