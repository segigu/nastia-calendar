...
function ensureEmojiPresent(text) {
  ...
  // Если эмодзи уже есть, просто обрезаем до 120
  if (EMOJI_REGEX.test(text)) {
    return truncateWithEllipsis(text, 120);
  }

  // Если эмодзи нет, добавляем щит
  // Обрезаем исходный текст до 115 символов (с запасом для " 🛡️")
  const shortened = text.length > 115 ? truncateWithEllipsis(text, 115) : text.trim();
  const withEmoji = `${shortened} 🛡️`;

  // Финальная проверка: если всё равно длиннее 120, обрезаем еще раз
  return withEmoji.length > 120 ? truncateWithEllipsis(withEmoji, 120) : withEmoji;
}
...
- ВАРЬИРУЙ темы и ситуации каждый день, не повторяй одни и те же паттерны.

Верни чистый текст без дополнительных пояснений.`;

  const systemPrompt = 'Ты Настя — язвительная подруга. Пиши по-русски, остро и поддерживающе. Переводи астрологию на житейский язык. Будь креативна, не повторяй шаблоны. Никаких форматов кроме чистого текста.';
...
function buildPrompt(type, context) {
  const base = `Ты — Настина лучшая подруга с жёстким, но поддерживающим сарказмом. Пиши по-русски дерзко, обращайся к Насте по-свойски (Настюш, Настён, Настёнка, Настёна, детка, Орлова).
Задача: push-уведомление для календаря цикла.
Формат:
- Заголовок (1-3 слова): вымышленное имя персонажа, связанное с темой фертильности/гормонов/беременности. ЗАПРЕЩЕНО: Игорь, Константин, Стас. Примеры: «Людмила Фертильная», «Олеся Овуляторовна», «Марфа Контрацептовна».
- Тело: ОДНА фраза, СТРОГО МАКС 105 символов (с эмодзи!). ⚠️ КРИТИЧНО: если длиннее 105 символов, текст обрежется многоточием на экране пользователя! Пиши законченную короткую фразу от лица персонажа. Юмор + сарказм. Допустим мат.
- ВАРЬИРУЙ формулировки каждый раз.

Сегодня: ${context.todayHuman}. Прогноз менструации: ${context.periodHuman}.`;

  switch (type) {
    case 'fertile_window':
      return `${base}
Ситуация: фертильное окно, до овуляции ${Math.abs(context.daysUntilOvulation)} ${context.daysWord}. Предупреди про риск беременности с юмором. Строго макс 105 символов!`;
    case 'ovulation_day':
      return `${base}
Ситуация: овуляция сегодня (пик фертильности). Саркастично предупреди о риске. Строго макс 105 символов!`;
    case 'period_forecast':
      return `${base}
Ситуация: до менструации ${Math.abs(context.daysUntilPeriod)} ${context.daysWord}. Напомни про ПМС, дай короткий совет. Строго макс 105 символов!`;
    case 'period_check':
    case 'period_start':
      return `${base}
Ситуация: прогноз менструации на ${context.periodHuman} (сегодня). Попроси проверить и отметить. Строго макс 105 символов!`;
    case 'period_waiting':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Поддержи саркастично. Строго макс 105 символов!`;
    case 'period_delay_warning':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Намекни про тест на беременность с сарказмом. Строго макс 105 символов!`;
    case 'period_confirmed_day0':
      return `${base}
Ситуация: Настя отметила начало менструации сегодня. Поддержи с юмором. Строго макс 105 символов!`;
    case 'period_confirmed_day1': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи с юмором, короткий совет. Строго макс 105 символов!`;
    }
    case 'period_confirmed_day2': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи, скажи что скоро легче. Строго макс 105 символов!`;
    }
    case 'birthday':
      return `Ты — Настина подруга с жёстким сарказмом. Push-уведомление: новый персонаж в заголовке (1-3 слова), тело ОДНА фраза строго макс 105 символов с 1-2 эмодзи.
Сегодня ${context.todayHuman}, Настюше ${context.birthdayAge}. Поздравь дерзко и тепло. ЗАПРЕЩЕНО: Игорь, Константин, Стас.`;
    default:
      return base;
  }
}
