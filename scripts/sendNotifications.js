...
- Тело МАКСИМУМ 110 символов (включая эмодзи!) с обращением к Насте. Пиши от лица персонажа из заголовка, будто он шлёт сообщение в чат. ЗАКОНЧЕННАЯ короткая фраза с юмором и сарказмом. Допустим умеренный мат.
- ВАРЬИРУЙ формулировки, советы и метафоры каждый раз. Не повторяй одни и те же слова и паттерны.
...
Сегодня: ${context.todayHuman}. Прогноз старта менструации: ${context.periodHuman}.`;
  ...
  switch (type) {
    case 'fertile_window':
      return `${base}
Ситуация: фертильное окно, до овуляции ${Math.abs(context.daysUntilOvulation)} ${context.daysWord}. Предупреди про риск беременности с юмором и сарказмом. МАКСИМУМ 110 символов!`;
    case 'ovulation_day':
      return `${base}
Ситуация: сегодня овуляция (пик фертильности). Саркастично предупреди о высоком риске беременности. МАКСИМУМ 110 символов!`;
    case 'period_forecast':
      return `${base}
Ситуация: до менструации ${Math.abs(context.daysUntilPeriod)} ${context.daysWord}. Напомни что ПМС скоро, дай совет как пережить (будь креативна). МАКСИМУМ 110 символов!`;
    case 'period_check':
    case 'period_start':
      return `${base}
Ситуация: прогноз на ${context.periodHuman} — то есть сегодня. Попроси проверить, началась ли менструация, и отметить в календаре. МАКСИМУМ 110 символов!`;
    case 'period_waiting':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord} после прогноза. Поддержи саркастично, скажи что организм сам разберётся. МАКСИМУМ 110 символов!`;
    case 'period_delay_warning':
      return `${base}
Ситуация: задержка ${context.daysPastPrediction} ${context.daysPastPredictionWord}. Саркастично намекни что неплохо бы сделать тест на беременность. МАКСИМУМ 110 символов!`;
    case 'period_confirmed_day0':
      return `${base}
Ситуация: Настя отметила начало менструации сегодня (день 1). Поддержи с юмором и сарказмом. МАКСИМУМ 110 символов!`;
    case 'period_confirmed_day1': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи с юмором, дай совет (будь креативна). МАКСИМУМ 110 символов!`;
    }
    case 'period_confirmed_day2': {
      ...
      return `${base}
Ситуация: ${dayWord} менструации. Поддержи, скажи что скоро полегчает. МАКСИМУМ 110 символов!`;
    }
    case 'birthday':
      return `Ты — Настина лучшая подруга с жёстким, но поддерживающим сарказмом. Пиши по-русски, формат push: новый персонаж в заголовке (1-3 слова) и тело МАКСИМУМ 110 символов с 1-2 эмодзи.
Сегодня ${context.todayHuman} и Настюше исполняется ${context.birthdayAge}. Поздравь дерзко и тепло. ЗАПРЕЩЕНО использовать имена: Игорь, Константин, Стас. Будь креативна!`;
    default:
      return base;
  }
}
